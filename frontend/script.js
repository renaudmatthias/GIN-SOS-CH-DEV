...
// Boutons pour fermer/annuler la fenêtre
document.getElementById("apm-close").addEventListener("click",  fermerFenetreAjout);
document.getElementById("apm-cancel").addEventListener("click", fermerFenetreAjout);

// Remet tous les champs de la fenêtre dans leur état initial (vide)
function reinitialiserFenetreAjout() {
  // Désélectionner tous les boutons de type
  for (const btn of document.querySelectorAll(".apm-type-btn")) {
    btn.classList.remove("selected");
  }
  document.getElementById("apm-name").value          = "";
  document.getElementById("apm-submit").disabled     = true; // bouton grisé au début
...
// Sélection du type de service (pompiers / police / hôpital)
// Un seul bouton peut être sélectionné à la fois
for (const btn of document.querySelectorAll(".apm-type-btn")) {
  btn.addEventListener("click", function() {
    // Désélectionner tous les boutons...
    for (const autreBtn of document.querySelectorAll(".apm-type-btn")) {
      autreBtn.classList.remove("selected");
    }
    // ...puis sélectionner celui qu'on vient de cliquer
    this.classList.add("selected");
    typeChoisi = this.dataset.type; // ex: "fire_station"
    verifierSiPretAEnregistrer();  // peut-on activer le bouton Enregistrer ?
  });
}
        ...
        lignesHTML +=
          '<tr data-id="' + point.id + '">' +
            ...
            '<td class="adm-cell-actions">' +
              ...
              '<button class="adm-btn adm-btn--edit"   onclick="ouvrirEdition(' + point.id + ',\'' + escapeHtml(point.name || "") + '\',' + point.x + ',' + point.y + ')">✏️</button>' +
              '<button class="adm-btn adm-btn--delete" onclick="supprimerPointAdmin(' + point.id + ')">🗑️</button>' +
            '</td>' +
          '</tr>';
      }
...
function ouvrirEdition(id, nom, x, y) {
  ...
  if (ligneSelectionnee) {
    ligneSelectionnee.classList.add("selected");
  }
}
